export function attachDebuggerHotkey(referenceAssemblies: string[]) {
  const hasPdbs = referenceAssemblies.some(x => /\.pdb$/.test(x));
  const isChrome = currentBrowserIsChrome();
  const hasDebuggingEnabled = hasPdbs && isChrome;
...
function currentBrowserIsChrome() {
